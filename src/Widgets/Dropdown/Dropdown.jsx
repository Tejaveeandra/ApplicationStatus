...
const dropdownsearchicon = (
  <svg width="19" height="20" viewBox="0 0 19 20" fill="none" xmlns="http://www.w3.org/2000/svg">
    <path
      ...
      fill="#A1A5B0"
      stroke="#A1A5B0"
      stroke-width="0.101865"
    />
  </svg>
);

const Dropdown = ({ dropdownname, results, onChange, value, name, error }) => {
  ...
  const filteredResults = Array.isArray(results)
    ? results.filter((item) => item.toLowerCase().includes(searchTerm.toLowerCase()))
    : [];
  ...
  return (
    <div className={styles.dropdown_wrapper} ref={dropdownRef}>
      ...
      <button
        type="button"
        className={`${styles.dropdown_button} ${error ? styles.dropdown_button_error : ""}`}
        onClick={handleToggle}
        onBlur={() => onChange({ target: { name, value }, type: "blur" })}
      >
      ...
      {isOpen && (
        <ul className={styles.dropdown_list}>
          <li className={styles.dropdown_search_list}>
            <div className={styles.dropdown_search_icon}>{dropdownsearchicon}</div>
            <input
              ref={inputRef}
              placeholder="Search"
